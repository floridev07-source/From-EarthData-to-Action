  ...
  const handleExportPDF = () => {
    ...
    const html = `
      <html>
        ...
        <body>
          <h1>Daily report - ${new Date(data.datetime).toLocaleString()}</h1>
          <p>${data.weather_summary}</p>
          <p>AQI Index: <strong style="color:${data.AQI_color_code}">${data.AQI_total_pred}</strong> — ${data.AQI_level_text}</p>
          <h2>Composition</h2>
          <ul>
          ...
          <table border="1" cellpadding="6" cellspacing="0">
            <thead><tr><th>Time</th><th>Temp (°C)</th><th>Wind (m/s)</th><th>Humidity (%)</th><th>Pressure (hPa)</th><th>AQI</th></tr></thead>
            <tbody>
  ...
  return (
    <div className="absolute inset-0 z-20 p-6 overflow-y-auto bg-black/60 backdrop-blur-md">
      ...
      <div className="max-w-4xl mx-auto space-y-4">
        <h2 className="text-white">Daily Report</h2>
        <div className="bg-white/5 border border-white/10 rounded-lg p-4 text-white/70 text-sm">
          <p className="mb-3">Summary: {data?.weather_summary || 'Unavailable'}</p>
          <p>AQI Index: <span style={{ color: data?.AQI_color_code }}>{data?.AQI_total_pred ?? '-'}</span> — {data?.AQI_level_text || '-'}</p>
        </div>
        <div className="flex items-center gap-3">
          <Button onClick={handleExportCSV} className="bg-white/10 border-white/20 text-white hover:bg-white/20">Export CSV</Button>
          <Button onClick={handleExportPDF} className="bg-white/10 border-white/20 text-white hover:bg-white/20">Export PDF</Button>
        </div>
        <div className="bg-white/5 border border-white/10 rounded-lg p-4 text-white/70 text-sm">
          <h3 className="text-white mb-2">Air composition</h3>
